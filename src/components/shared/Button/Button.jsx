...
function Button({label ,type , className , disabled = false , onClick}) {
    const bg = {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={' transition-all duration-300 text-white cursor-pointer shadow-xs ' + bg[type]+ " " + className}>
            {label}
    </button>
  )
}
